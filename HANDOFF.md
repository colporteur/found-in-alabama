...
**Phase 5 (next): rollback + history browser.** Before/after snapshots have been captured since Phase 1 (jobs table: before/after jsonb, rolledBack flag reserved). Build: per-item / per-batch / last-24h rollback actions that ReviseItem the `before` values back, plus a history browser across batches. Careful with description rollbacks (full-HTML replace from a snapshot capped at 20k chars — snapshot may be truncated for very long descriptions; check length before offering rollback) and specifics rollbacks (before = empty values → rollback means REMOVING those specifics via full-set write).
