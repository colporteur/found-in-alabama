...
**Phase 2 (next): Item Specifics fill.** First LLM op — Gemini Flash or GPT-4o-mini through `callLlm()`. Requires setting `GEMINI_API_KEY` or `OPENAI_API_KEY` in Vercel + `.env.local`. Needs a GetItem ItemSpecifics reader and a ReviseItem ItemSpecifics writer in `lib/ebay/calls.ts`.
