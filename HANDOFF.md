...
**Phase 3 (next): title + description remix with expert guides.** Guide loader + `content/expert-guides/` manifest (decision #5), prompt caching via `cacheableSystem` (already supported in callLlm), Haiku 4.5 for titles / Sonnet 5 for descriptions, per-op model config. Mind eBay's 80-char title limit and Hard-Rule-6-style guardrails (no shipping/discount/return language changes) from the Nifty extension's Expert Mode.
