...
**Phase 4 (next): price research reprice via APR.** Wire `callHttpService` to APR's submit/poll flow (`POST /api/v1/research` → poll `GET /research/{job_id}`, X-API-Key header, tunnel URL https://aprapi.dev). Env needed: `APR_API_URL`, `APR_API_KEY`. Poll GETs pass `billable: false`; only the submit logs the $0.03 pass-through. Mind the 45s tick budget vs APR's ~60s job time — submit in one tick, poll in later ticks (persist job_id in enhance_jobs.result).
